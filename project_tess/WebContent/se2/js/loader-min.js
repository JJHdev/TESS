...
nhn.FindReplace = jindo
		.$Class({
			...
			findNext : function(a, b, d, e) {
				...
				if (this.window.find) {
					var g = false;
					return this.window.find(a, b, d, g, e)
				}
...
nhn.BrowserSelection = function(b) {
	this.init = function(c) {
		this._window = c || window;
		this._document = this._window.document
	};
	this.init(b);
	var a = jindo.$Agent().navigator();
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
	...
	$ON_REGISTER_UI_EVENT : function(b, c, e, a) {
		if (!this.aUICmdMap[b]) {
			this.aUICmdMap[b] = []
		}
		this.aUICmdMap[b][this.aUICmdMap[b].length] = e;
		var d = this.htUIList[b];
		if (!d) {
			return
		}
		this.oApp.registerBrowserEvent(d.firstChild, c, e, a)
	},
...
nhn.husky.SE_EditingArea_TEXT = jindo.$Class({
	...
	textToIr : function(g) {
		...
		var b = g, e = null;
		var h = null, d = 0, f = null;
		f = b.match(new RegExp(this.sRxConverter));
		if (f !== null) {
			b = b.replace(f[0], "")
		}
		b = this.htmlSpecialChars(b);
		h = b.split("\n");
		d = h.length;
		for ( var a = 0; a < d; a++) {
			e = jindo.$S(h[a]).trim().$value();
			if (a === d - 1 && e === "") {
				break
			}
			if (e !== null && e !== "") {
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo
		.$Class({
			...
			_getResizeHeight : function() {
				var d = this.getDocument().body, f, b = [ "width",
				...
				var a = jindo.$Element(d), c = b.length, e = {};
				while (c--) {
					e[b[c]] = a.css(b[c])
				}
				if (e.lineHeight.indexOf("px")) {
			...
			_onIEBeforeDeactivate : function(b) {
				...
				var a = this.getDocument().selection;
				var c = a.createRange();
				if (c.parentElement && c.parentElement()
						&& c.parentElement().tagName == "INPUT") {
			...
			$ON_PASTE_HTML : function(b, g, e) {
				...
				if (j.ie && j.nativeVersion >= 9 && document.documentMode >= 9) {
					b = b + unescape("%uFEFF")
				}
				c.pasteHTML(b);
				if (!j.ie) {
					var f = c.placeStringBookmark();
				...
				if (b.indexOf("<img") > -1) {
					var l = c.startContainer, h, d, k, a;
			...
			_pageUp : function(c) {
				...
				var d = jindo.$Document(this.oApp.getWYSIWYGDocument())
						.scrollPosition();
				var a;
				if (d.top <= b) {
					a = 0
		...
		.$Class(
				{
					...
					_assignHTMLElements : function(b) {
						...
						if (a) {
							this.bUseFlashModule = a.bUseFlashModule || false;
							this.URL_COLOR_ADD = a.addColorURL
									|| "http://api.se2.naver.com/1/colortable/TextAdd.nhn?text_data=";
							this.URL_COLOR_DELETE = a.delColorURL
									|| "http://api.se2.naver.com/1/colortable/TextDelete.nhn?text_data=";
					...
					$ON_APPLY_COLOR : function(b) {
						if (this.elInputColorCode.value.indexOf("#") == -1) {
							this.elInputColorCode.value = "#"
									+ this.elInputColorCode.value
						}
						if (b == this.elOkBtn) {
							if (!this.rxColorPattern
									.test(this.elInputColorCode.value)) {
								this.elInputColorCode.value = "";
								alert(this.oApp
							...
							this.oApp.exec("COLOR_PALETTE_APPLY_COLOR", [
									this.elInputColorCode.value, true ]);
							return
						}
						var a = jindo
								.$Element(b.parentNode.parentNode.parentNode);
						this.elInputColorCode.value = b.title;
						if (a.hasClass("husky_se2m_color_palette")) {
							this.oApp.exec("COLOR_PALETTE_APPLY_COLOR", [
									this.elInputColorCode.value, false ])
						} else {
							if (a.hasClass("husky_se2m_color_palette_recent")) {
								this.elOkBtn.click()
							}
						}
					},
					...
					$ON_COLOR_PALETTE_APPLY_COLOR : function(f, a) {
						a = (!a) ? false : a;
						if (this.rxRGBColorPattern.test(f)) {
							var b = function(g) {
								var h = parseInt(g, 10).toString(16);
								if (h.length < 2) {
									h = "0" + h
								}
						...
						if (this.bUseFlashModule && !!a) {
							this.oApp.exec("ADD_RECENT_COLOR", [ f ])
						}
						this.oApp.exec(this.sCallbackCmd, [ f ])
					},
					...
					$ON_ADD_RECENT_COLOR : function(b) {
						if (this.bUseFlashModule
								&& !jindo.$Ajax.SWFRequest.activeFlash) {
							alert(this.oApp
									.$MSG("SE.SE2M_ColorPalette.failedToLoadFlash"));
							return
						} else {
							if (!this.bUseFlashModule) {
								return
						...
						var h = this.elRecentColor.childNodes;
						for ( var c = 0, j = h.length; c < j; c++) {
							if (typeof h[c] == "undefined") {
								break
							}
							if (h[c].nodeType == 1) {
								var e = jindo.$$.getSingle("button", h[c]);
								if (e.title != b) {
									continue
								}
								this._ajaxDeleteColor(e.title);
								jindo.$Element(h[c]).leave();
								this.nRecentColorNum--;
								break
							}
						}
						this.aRecentColor.unshift(b);
						var a = this.sRecentColorTemp.replace(/\{RGB_CODE\}/gi,
								b);
						jindo.$Element(this.elRecentColor).prepend(a);
						setTimeout(
								jindo.$Fn(this._ajaxSendColor, this).bind(b),
								17);
						this.nRecentColorNum++;
						if (this.nRecentColorNum > this.nLimitRecentColor) {
							...
							do {
								g++;
								d = this.elRecentColor.childNodes[this.elRecentColor.childNodes.length
										- g]
							} while (this.elRecentColor.childNodes[this.elRecentColor.childNodes.length
									- g].nodeType != 1);
							var e = jindo.$$.getSingle("button", d);
							var k = e.title;
							jindo.$Element(d).leave();
							this._ajaxDeleteColor(k);
							var f = jindo.$A(this.aRecentColor).refuse(k);
							this.aRecentColor = f.$value();
							this.nRecentColorNum--
						}
						var f = jindo.$A(this.aRecentColor).unique();
						this.aRecentColor = f.$value();
						this.elColorPaletteLayerRecent.style.display = "block"
					},
					_ajaxDeleteColor : function(a) {
						var b = this.URL_COLOR_DELETE + escape(a);
						new jindo.$Ajax(b, {
							type : "flash",
							sendheader : false
						}).request()
					},
					_ajaxSendColor : function(a) {
						var b = this.URL_COLOR_ADD + escape(a);
						new jindo.$Ajax(b, {
							type : "flash",
							sendheader : false
						}).request()
					...
					_ajaxRecentColor : function(c) {
						if (jindo.$Ajax.SWFRequest.activeFlash) {
							var a = this.URL_COLOR_LIST;
							var b = new jindo.$Ajax(a, {
								type : "flash",
								sendheader : false,
					...
					_ajaxRecentColorCallback : function(e) {
						...
						b = b.reverse();
						b = jindo.$A(b).refuse("").$value();
						for ( var a = 0, c = b.length; a < c; a++) {
							if (a == this.nLimitRecentColor) {
								break
							}
							if (b[a].indexOf("#") > -1) {
								var d = this.sRecentColorTemp.replace(
										/\{RGB_CODE\}/gi, b[a]);
								jindo.$Element(this.elRecentColor).append(d);
								this.aRecentColor.push(b[a])
							}
						}
						this._initRecentColor()
					},
					_initRecentColor : function() {
						var c = this.elRecentColor.childNodes;
						var d = 0;
...
nhn.husky.SE2M_ExecCommand = jindo
		.$Class({
				...
				if (c.tagName === "LI") {
					if (c.previousSibling
						...
						if (c.nextSibling
							...
							var g = [ c ];
							for ( var d = 0, e = c.nextSibling.childNodes.length; d < e; d++) {
					...
					var f = c.parentNode.cloneNode(false);
					c.parentNode.insertBefore(f, c);
					f.appendChild(c);
					return
				}
				var h = parseInt(c.style.marginLeft);
				if (!h) {
					h = 0
				}
			...
			_outdentMargin : function(d) {
				...
				if (d.tagName === "LI") {
					...
					if (!d.parentNode.tagName.match(/OL|UL/)) {
						var f = d.parentNode;
						h = d;
						var a = this.oApp.getWYSIWYGDocument();
						f = a.createElement("P");
						h = null;
				...
				var g = parseInt(d.style.marginLeft);
				if (!g) {
					g = 0
				}
			...
			_insertBlankLine : function() {
				...
				while (a) {
					if (a.tagName == "BLOCKQUOTE") {
						this.elP1 = this.oApp.getWYSIWYGDocument()
								.createElement("P");
						a.parentNode.insertBefore(this.elP1, a);
						this.elP2 = this.oApp.getWYSIWYGDocument()
								.createElement("P");
			...
			_fixCorruptedBlockQuote : function(c) {
				var h = this.oApp.getWYSIWYGDocument().getElementsByTagName(c);
				var g;
				for ( var f = 0, k = h.length; f < k; f++) {
					if (h[f].firstChild && h[f].firstChild.tagName == c) {
						g = h[f];
						break
					}
				}
				...
				var d = g.parentNode;
				var a = this._getPosIdx(g);
				var b = this.oApp.getWYSIWYGDocument().createElement("DIV");
				b.innerHTML = g.outerHTML.replace("<" + c, "<BLOCKQUOTE");
				g.parentNode.insertBefore(b.firstChild, g);
				g.parentNode.removeChild(g);
				var j = d.childNodes[a];
				var e = j.getElementsByTagName(c);
				...
				oSelection.selectNodeContents(j);
		...
		.$Class({
			...
			$ON_APPLY_FONTCOLOR : function(a) {
				...
				if (b.ie || b.firefox) {
					...
					this.oApp.exec("EXECCOMMAND", [ "ForeColor", false, a, {
						bDontAddUndoHistory : c
					} ]);
					if (c) {
						this.oApp.exec("RECORD_UNDO_ACTION", [ "FONT COLOR", {
							bMustBlockElement : true
						} ])
					}
				}
...
nhn.husky.SE2M_Emoticon = jindo
		.$Class({
			name : "SE2M_Emoticon",
			$ON_MSG_APP_READY : function() {
				var a = nhn.husky.SE2M_Configuration.SE2M_Emoticon;
				if (a) {
					this.sImgBaseURL = a.sImgBaseURL || "img/emoticon/"
				}
				this.oApp.exec("REGISTER_UI_EVENT", [ "emoticon", "click",
						"TOGGLE_EMOTICON_LAYER" ])
			},
			_assignHTMLObjects : function(b) {
				this.elDropdownLayer = jindo.$$.getSingle(
						"DIV.husky_se2m_emoticon_layer", b);
				var a = jindo.$$.getSingle("UL.se2_imo_tab",
						this.elDropdownLayer);
				this.aLabel = jindo.$$(">LI", a);
				this.aListUL = jindo.$$("DIV>UL", a);
				this.bListDrawn = []
			},
			$LOCAL_BEFORE_FIRST : function() {
				this.bIE = jindo.$Agent().navigator().ie;
				this._assignHTMLObjects(this.oApp.htOptions.elAppContainer);
				this.oApp.exec("SET_EMOTICON_LIST", []);
				this.aEmoticonList = nhn.husky.SE2M_EmoticonList;
				for ( var a = 0; a < this.aLabel.length; a++) {
					var b = jindo.$Fn(this.oApp.exec, this.oApp).bind(
							"CHANGE_EMOTICON_SET", [ a ]);
					jindo.$Fn(b, this).attach(this.aLabel[a].firstChild,
							"mousedown")
				}
				this.oApp.registerBrowserEvent(this.elDropdownLayer, "click",
						"EVENT_EMOTICON_CLICKED", []);
				this._drawEmoticonList(0)
			},
			$ON_CHANGE_EMOTICON_SET : function(b) {
				for ( var a = 0; a < this.aLabel.length; a++) {
					if (jindo.$Element(this.aLabel[a]).hasClass("active")) {
						...
						this.aListUL[a].parentNode.style.display = "none"
					}
				}
				this._drawEmoticonList(b);
				jindo.$Element(this.aLabel[b]).addClass("active");
				this.aListUL[b].parentNode.style.display = "block"
			},
			$ON_EVENT_EMOTICON_CLICKED : function(c) {
				var b = nhn.husky.SE2M_Utils.findAncestorByTagName("BUTTON",
						c.element);
				if (!b || b.tagName != "BUTTON") {
					return
				}
				if (b.parentNode.tagName != "LI") {
					return
				}
				if (!b.firstChild || b.firstChild.firstChild.tagName != "IMG") {
					return
				}
				c.stop();
				var a = jindo.$$.getSingle("IMG", b);
				this.oApp.exec("TOGGLE_EMOTICON_LAYER", []);
				this.oApp.exec("PASTE_HTML", [ "<img src=" + a.src + ">" ]);
				this.oApp.exec("FOCUS")
			},
			$ON_TOGGLE_EMOTICON_LAYER : function() {
				...
				this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [
						...
						[ "emoticon" ], "DESELECT_UI", [ "emoticon" ] ]);
				this.oApp.exec("MSG_NOTIFY_CLICKCR", [ "emoticon" ])
			},
			_drawEmoticonList : function(c) {
				if (this.bListDrawn[c]) {
					return
				}
				this.bListDrawn[c] = true;
				this.nCurEmoticonSet = c;
				var a = this.aEmoticonList[c].length;
				var g = new Array(a);
				this.aListUL[c].innerHTML = "";
				var f = this.sImgBaseURL;
				var b, e;
				for ( var d = 0; d < a; d++) {
					g[d] = jindo.$("<LI>");
					if (this.bIE) {
					...
					e = jindo.$("<SPAN>");
					e.innerHTML = "<img _se2_emoticon_idx='" + d + "' src='"
							+ f + this.aEmoticonList[c][d]
							+ "' style='width:19px; height:19px'>";
					b.appendChild(e);
					g[d].onmouseover = function() {
						this.className = "hover"
					};
					g[d].onmouseout = function() {
						this.className = ""
					};
					g[d].appendChild(b);
					this.aListUL[c].appendChild(g[d])
				}
			}
		});
nhn.husky.SE2M_EmoticonList = [];
nhn.husky.SE2M_EmoticonData = jindo
		.$Class({
			name : "SE2M_EmoticonData",
			sLang : "ko_KR",
			$init : function(a) {
				if (!!a && a != "") {
					this.sLang = a
		...
		.$Class({
			...
			$ON_TABLE_INSERT : function() {
				...
				this.nTableWidth = c.offsetWidth
						- parseInt(k.css("marginLeft"), 10)
						- parseInt(k.css("marginRight"), 10);
...
nhn.husky.SE2M_QuickEditor_Common = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				if (a) {
					this._bUseFlashModule = (!!a.common && typeof a.common.bUseFlashModule !== "undefined") ? a.common.bUseFlashModule
							: true
				}
				if (!this._bUseFlashModule
						|| !jindo.$Ajax.SWFRequest.activeFlash) {
			...
			$ON_MSG_BEFOREUNLOAD_FIRED : function() {
				if (!this._environmentData || !this._bUseFlashModule
						|| !jindo.$Ajax.SWFRequest.activeFlash) {
					return
				}
			...
			getElementPosition : function(e, b) {
				...
				if (e) {
					...
					d = {
						top : parseInt(b.style.top) - this.nYGap,
						left : parseInt(b.style.left) - this.nXGap
					};
					g = 0;
					c = 0
				}
...
// 리사이징 시				
				var a = '<img src="{=sOriginalImageURL}" title="{=sName}" width="{=sWidth}" height="{=sHeight}" >';
				if (b.bNewLine) {
