...
//						}
//					});
					// [2014.11.17 LCS 수정] 메시지 dialog를 동일한 형태로 수정
                    msgAlert($("#dialog").text());
				}
...
//				if (success && _rtnFunc != undefined)	eval(_rtnFunc)(result);
				var funcObj = null;
                if(_rtnFunc != undefined && _rtnFunc != null && _rtnFunc.length > 0) {
                    if( typeof _rtnFunc == "string"){
                        funcObj = eval(_rtnFunc);
                    }
                    else if( typeof _rtnFunc == "function"){
                        funcObj = _rtnFunc;
                    }
...
//					if (success && _rtnFunc != undefined)	eval(_rtnFunc)(result);
	    ...
	    msgAlert($( "#dialogSelectRow" ).text().trim());
	}
}
...
function submitSave(useropt) {
    ...
    if ( row != null ) {
        ...
        if(_msg) {
            nConfirm(_msg, null, function(isConfirm){
                if(isConfirm){
                    _submitSave(useropt, _params) ;
                }
            });
        }else{
            _submitSave(useropt, _params) ;
...
function _submitSave(useropt, _params) {
    var _gridName       = useropt.gridName;
    var _url                = useropt.url;
    var _userData       = useropt.userData;
    var _rtnFunc            = useropt.rtnFunc;
	...
	for (var i = 0; i < id.length; i++) {
		...
		for (var ii = 0; ii < cols.length; ii++ ) {
			if (cols[ii].name != 'cb') {
				...
				// json 데이터중에 쌍따옴표 있을경우 에러처리 ntarget : 2014-12-17
				colValue = $.toJSON(colValue);
