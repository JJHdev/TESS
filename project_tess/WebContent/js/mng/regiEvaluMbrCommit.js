...
//  window.history.forward(0);
  ...
  $(".userChkDuplBtn").click(function(){
	    ...
	    console.log("dasads??? : " + fn_checkType(userIdObj));
...
//  	openDaumPostcode();
  ...
  //사무실 번호 입력 이벤트 연결
  $("#telphoneNo1,#telphoneNo2,#telphoneNo3").change(function(){
      fn_uscm_setFullTelphoneNo();
  });
...
var SAVE_URL      = ROOT_PATH+"/mng/saveCommitMembInfo.do" ;
var LIST_URL      = ROOT_PATH+"/mng/listEvaluMbrMgmt.do" ;
...
//사무실 번호 설정.
function fn_uscm_setFullTelphoneNo() {
    var phoneNo1Val = $("#telphoneNo1").val();
    var phoneNo1Va2 = $("#telphoneNo2").val();
    var phoneNo1Va3 = $("#telphoneNo3").val();

    $("#telphoneNo").val(phoneNo1Val+"-"+phoneNo1Va2+"-"+phoneNo1Va3);
}
...
function doValidation(){
    ...
    BIZComm.getCheckObjects($("#model")).each(function(){
        ...
        if(chkObjNm=="userId"){ 
        	var etcChkObj = chkObj.parents().find("input:hidden[name=userIdDuplChkYn]");
        ...
        if(chkObjNm=="telphoneNo2"&&isValid){ 
        	if(!fn_checkType(chkObj, chkObjNm)){ isValid = false; return false; };
        }
        if(chkObjNm=="telphoneNo3"&&isValid){ 
        	if(!fn_checkType(chkObj, chkObjNm)){ isValid = false; return false; };
        }
