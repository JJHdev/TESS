...
function goRegi() {
    ...
    // submit
    BIZComm.dySubmit({
        url : ROOT_PATH+"/evalu/regiEvaluDpthMgmt.do",
        userParam      : userParam
    });
}
...
function goView(arrParam) {
    ...
    // submit
    BIZComm.dySubmit({
        url : ROOT_PATH+"/evalu/viewEvaluInptDpthMgmt.do",
        userParam      : userParam
    });
}
...
function tab2(){
	var form = document.model;
	form.action="/evalu/viewEvaluDpthMgmt.do";
	form.submit();
}
...
function reporting(){
	// 파라메터 값이 NULL인 경우 쿼리에서 임의 문자 escape로 받아 처리.  
	 ...
	 for(var i=0; i < userIdSelected.length; i++){
		 ...
		 newWindow.location.href="http://125.7.243.212:8888/AIViewer55/reporttest/TESS/OP_DeepEvalu_2014.jsp?busiNo="+repParam_evaluBusiNo+"&busiNm="+repParam_evaluBusiNm
