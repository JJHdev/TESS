...
var SEARCH_URL    = ROOT_PATH + "/evalu/getEvaluStgMgmt.do" ;
var REGI_URL		= ROOT_PATH + "/evalu/regiEvaluStgMgmt.do" 
...
function setGridInfoByMenu(){
        ...
        colNames = ["지역", "사업명", "사업기간", "사업의 유형", "총 사업비(원)", "평가단계", "평가일", "관광개발사업번호", "사업상태코드", "사업유형코드", "평가단계 코드"];
        colModel = [
                    ...
                    {name:'convEvaluDate',    index:'evalu_date',    width: 90, editable:false, align:"center"},
...
function getSearchPostData(){
    var postData = {
            ...
            srchBusiSttDate : $("#srchBusiSttDate").val(),
    }
...
function setSearchCond(){
    ...
    $("#srchBusiStage").val($("#srchBusiStageTemp").val());
    $("#srchBusiSttDate").val($("#srchBusiSttDateTemp").val());
...
// 입력/수정 화면으로 이동
function goView(evaluBusiNo, evaluStage){
	
    var mode;
    // 평가단계 값이 없으면 수정
    if(isEmpty(evaluStage)) {
        mode = EVALUComm.mode.regi;
    }
    // 평가단계 값이 있으면 입력
    else{
    	 mode = EVALUComm.mode.updt;
    }
