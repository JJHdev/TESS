 ...
 * @return
 */
function getRadioValue2(object) {
	var rtn = '';
...
//    var bizRegNo = this.match(pattern);
    ...
    //TODO [shlee] 테스트데이터 SKIP 처리(나중에 막을것)
    if (bizRegNo == "1111111111")
        return true;
...
$.fn.toCalendarField = function( args ) {
    ...
    // 달력에 클릭 이벤트를 바인딩한다.
    //※ 참고사항
    // 달력사용시엔 반드시 스타일시트에 ibsheet 가 포함되어야 한다.
    // ex) <app:layout mode="stylescript" type="ibsheet" />
    ...
    //    
    //    IBShowCalendar(element.value, options);
...
$.fn.toFloatField = function() {
    ...
    // 필드에 입력된 값을 포맷한다.
//    if (!isEmpty(this.val())) {
//         this.val(addCommaStr(delCommaStr(this.val())));
//    }
...
// [20141211 LCS 추가] 주소 검색 POPUP 여는 함수
function openFindJuso(){
    //newWindow(ROOT_PATH+'/jusoPopup.jsp', 'findPostNoAddressPop', 530, 420, "yes");
	window.open(ROOT_PATH+'/jusoPopup.jsp', 'findPostNoAddressPop', "width=570, height=420, scrollbars=yes, resizable=yes");
}
