...
function doSave() {
	...
	var docType01 = $("input[name=docType01]").is(":checked");
	var docType02 = $("input[name=docType02]").is(":checked");
	var docType03 = $("input[name=docType03]").is(":checked");
		...
		if(docType02 == true) {
			if(docType03 == true) {
				...
				nConfirm("제출하시겠습니까?", null, function(isConfirm){
					...
					var params = {"evaluBusiNo": evaluBusiNo, "evaluGubun": evaluGubun, "evaluStage": evaluStage, "userId": userId, "agreeYn": agreeYn};
...
//						            contentType:"application/json; text/html; charset=utf-8",
				...
				jAlert("개인정보 수집 및 이용 동의서에 동의하세요.");
			}
		} else {
			jAlert("청탁금지서약서에 동의하세요.");
		}
	} else {
		jAlert("보안각서에 동의하세요.");
	}
