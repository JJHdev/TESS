...
***  -----------------------------------------------------------------------------------------
*** 1.0         2014-08-16                      LCS                 First Coding.
*****************************************************************************************/
...
//        var calendarClassNm        = (args.calendarClassNm)? args.calendarClassNm:DYCom.dfCalendarClassNm;
        ...
        try {
           ...
           //var newRow = tbody.find('tr:eq('+(srcIdx)+')').clone(true).appendTo(tbody);
...
//        var beforeRemove           = args.beforeRemove;
//        var buildFileObjHtmlString = args.buildFileObjHtmlString;
//        var changeFile             = args.changeFile;
...
DYCom = {
    ...
    removeUnit : function(args) {
        ...
        if(args!=undefined && args != null) {
          ...
          //삭제로우 기준값 추가 20160425	//값 없으면 기본값 1
            stdDelRow       			= (args.stdDelRow      )? args.stdDelRow      :1;
        }
        ...
        if(remObj){
          ...
          //if(unitCnt > stdDelRow || isRemoveLastUnit) {
            
            //삭제로우 기준값으로 변경 20160425
            if(unitCnt > stdDelRow || isRemoveLastUnit) {
                remObj.remove();
                rtnObj = null;
            }else{
    ...
    setControlInit : function (args){
        ...
        try {
            ...
            //[20130905 added by lcs] remove disabled attr-
            //맨 마지막 row를 삭제할 때 disabled된 상태가 유지되는 문제해결
            //jDyUnitobj.find(":disabled").prop("disabled",false);
    ...
    isCheckObjects  : function(unitObj) {
        ...
        var isCheck = true;
        DYCom.getCheckObjects(unitObj).each(function(){
//            if($(this).val().trim() != ""){   // ie8에서 오류 (.trim()을 인식 못함)
            if( $.trim($(this).val()) != ""){
                inObjCnt ++;
                return false; // break
            }
