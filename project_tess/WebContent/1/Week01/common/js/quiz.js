...
$(document).ready(function () {
    ...
    $('.quiz_confirm').bind('click', function () {
        ...
        } else if ($(obj).hasClass('quizComplete')) {
            confirm = 3;
        } else if ($(obj).hasClass('line_quiz')) {
            confirm = 2;
            if (quizComplete) { confirm = 3; }
        } else {
            confirm = 2;
            if (quizt[qi] == null) {
                $(obj).parents('form').find('.inputText, .text').each(function () {
                    if ($(this).val().replace(/\s/g, "") == "") { confirm = 2; return false; } else { confirm = 3; }
                });
        ...
        if (confirm == 0) {
            ...
            $("body").append('<embed src="data/an2.wav" hidden="true" autostart="true" />');
        } else {
            $("body").append('<embed src="data/x.wav" hidden="true" autostart="true" />');
        }

        if (confirm == 0 || confirm == 3 || falsecnt < 1) {
    ...
    $("[drop_value]").droppable({
        ...
        drop: function (event, ui) {
            if ($(this).attr('drop_value') == dragValue) {
                ...
                $("body").append('<embed src="data/o.wav" hidden="true" autostart="true" />');
                if ($('[drag_value]:not(".dragvalue_true")').length == 0) $(this).parents('.dragdrop_containment').siblings('.quiz_confirm').addClass('quizComplete');
            } else {
