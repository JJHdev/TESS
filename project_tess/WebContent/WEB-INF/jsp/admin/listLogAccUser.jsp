<%
 /****************************************************************************************
*** ID                  : listLogAccUser.jsp
*** Title               :  [관리자-사용자별접속통계] 리스트 화면
*** Description         :  [관리자-사용자별접속통계] 리스트 화면
***
*** -----------------------------    Modified Log   --------------------------------------
...
<form:form commandName="model" name="model" id="model">
    ...
    <%-- 검색조건 --%>
    <div id="srchCondArea">
        <input type="hidden" name="srchDateType"    id="srchDateType"  />
        <input type="hidden" name="srchFromDate"    id="srchFromDate"  />
        <input type="hidden" name="srchToDate"      id="srchToDate"  />
        <input type="hidden" name="srchUserId"      id="srchUserId"  />
        <input type="hidden" name="srchUscmType"    id="srchUscmType"  />
    </div>
    ...
    <div style="width:90%; padding:0 0 0 10px;">
        <table class="style_search_01">
            ...
            <tr>
                ...
                <td>
                    <!--
                    <select name="srchDateTypeTemp" id="srchDateTypeTemp" style="width:120px;" title="검색기간 구분">
                ...
                <th><label for="srchUserIdTemp">아이디</label></th>
                <td>
                    <input type="text" id="srchUserIdTemp" name="srchUserIdTemp" style="width:200px;" title="아이디"/>
                </td>
            </tr>
            <tr>
                <th>사용자구분</th>
                <td colspan="1">
                    <select name="srchUscmTypeTemp" id="srchUscmTypeTemp" style="width:120px;" title="사용자구분">
                        <option value="">::전체::</option>
                        <c:forEach items="${uscmTypeComboList }" var="ucsm" varStatus="status">
                            <option value="${ucsm.code }" label="${ucsm.codeNm }"><c:out value="${ucsm.codeNm }"/></option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
